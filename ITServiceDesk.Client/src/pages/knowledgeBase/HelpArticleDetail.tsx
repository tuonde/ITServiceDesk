...
export const HelpArticleDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchArticle = async () => {
            ...
            try {
                ...
                toast.error('Makale yüklenemedi veya bulunamadı.');
                navigate('/help');
            } finally {
                setLoading(false);
            }
    ...
    const getArticleTypeBadge = (type: KbArticleType) => {
        switch (type) {
            ...
            case KbArticleType.Troubleshooting: return <span className="px-3 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700">Çözüm</span>;
            case KbArticleType.Procedure: return <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700">Prosedür</span>;
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 sm:p-8 pb-16">
            ...
            <article className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mb-8">
                <div className="p-8 md:p-12 border-b border-slate-100 bg-slate-50/50">
                    ...
                    <div className="flex flex-wrap items-center gap-6 text-sm text-slate-500 font-medium">
                        ...
                        <div className="flex items-center gap-2" title="Yayınlanma Tarihi">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            {new Date(article.createdAt).toLocaleDateString('tr-TR')}
                        </div>
                        <div className="flex items-center gap-2" title="Okunma Sayısı">
            ...
            <div className="bg-emerald-50 rounded-2xl p-8 text-center border border-emerald-100 shadow-sm mb-12">
                {feedbackSubmitted ? (
                    <div className="flex flex-col items-center justify-center animate-fade-in-up">
                        ...
                        <h3 className="text-lg font-bold text-emerald-800">Geri bildiriminiz alındı!</h3>
                        <p className="text-emerald-600 mt-1">İçeriğimizi geliştirmemize yardımcı olduğunuz için teşekkür ederiz.</p>
                    </div>
                ) : (
                    <>
                        <h3 className="text-lg font-bold text-slate-800 mb-6">Bu makale faydalı oldu mu?</h3>
            ...
            <div className="bg-slate-800 rounded-2xl p-8 flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left shadow-lg">
                <div>
                    <h3 className="text-xl font-bold text-white mb-1">Bu makale sorununuzu çözmedi mi?</h3>
                    <p className="text-slate-300">Teknik destek ekibimize yeni bir talep ileterek doğrudan yardım alabilirsiniz.</p>
                </div>
                <button 
