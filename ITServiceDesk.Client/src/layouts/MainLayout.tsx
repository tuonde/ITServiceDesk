...
const MainLayout: React.FC = () => {
  ...
  else if (isAuditLogsPage) pageTitle = '';
  else if (isSettingsPage) pageTitle = '';
  else if (isReportsPage) pageTitle = '';
  else if (location.pathname.startsWith('/inventory')) pageTitle = '';
  else if (location.pathname.startsWith('/profile')) pageTitle = '';
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col font-sans text-slate-800 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Main Content */}
        <main className={`flex-1 flex flex-col relative overflow-hidden bg-slate-50/50 ${!isAdmin ? 'mb-16 sm:mb-0' : ''}`}>
      ...
      {/* Mobile Bottom Navigation for Users and Technicians */}
      {!isAdmin && (
        <nav className="sm:hidden fixed bottom-0 inset-x-0 bg-white/90 backdrop-blur-md border-t border-slate-200/60 flex items-center justify-around z-50 h-16 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          ...
          <Link to="/my-inventory" className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-all ${location.pathname === '/my-inventory' ? 'text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            <span className="text-[10px] font-bold tracking-wide">Zimmetlerim</span>
          </Link>
